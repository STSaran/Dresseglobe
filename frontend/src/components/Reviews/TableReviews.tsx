...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/reviews/reviewsSlice'
...
const TableSampleReviews = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { reviews, loading, count, notify: reviewsNotify, refetch } = useAppSelector((state) => state.reviews)
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `reviews`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
